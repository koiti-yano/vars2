...
#' \sQuote{AB-model}) by using a scoring algorithm or by directly minimising
#' the negative log-likelihood with \command{optim()}.
#'
#' Consider the following structural form of a k-dimensional vector
#' autoregressive model:
#'
#' \deqn{ A \bold{y}_t = A_1^*\bold{y}_{t-1} + \ldots + A_p^*\bold{y}_{t-p} +
#' C^*D_t + B\bold{\varepsilon}_t }
#'
#' The coefficient matrices \eqn{(A_1^* | \ldots | A_p^* | C^*)} might now
#' differ from the ones of a VAR (see \code{?VAR}). One can now impose
#' restrictions on \sQuote{\code{A}} and/or \sQuote{\code{B}}, resulting in an
#' \sQuote{A-model} or \sQuote{B-model} or if the restrictions are placed on
#' both matrices, an \sQuote{AB-model}. In case of a SVAR \sQuote{A-model},
#' \eqn{B = I_K} and conversely for a SVAR \sQuote{B-model}. Please note that
#' for either an \sQuote{A-model} or \sQuote{B-model}, \eqn{K(K-1)/2}
#' restrictions have to be imposed, such that the models' coefficients are
#' identified. For an \sQuote{AB-model} the number of restrictions amounts to:
#' \eqn{K^2 + K(K-1)/2}.\cr For an \sQuote{A-model} a \eqn{(K \times K)} matrix
#' has to be provided for the functional argument \sQuote{\code{Amat}} and the
#' functional argument \sQuote{\code{Bmat}} must be set to \sQuote{\code{NULL}}
#' (the default). Hereby, the to be estimated elements of \sQuote{\code{Amat}}
#' have to be set as \sQuote{\code{NA}}. Conversely, for a \sQuote{B-model} a
#' matrix object with dimension \eqn{(K \times K)} with elements set to
#' \sQuote{\code{NA}} at the positions of the to be estimated parameters has to
#' be provided and the functional argument \sQuote{\code{Amat}} is
#' \sQuote{\code{NULL}} (the default). Finally, for an \sQuote{AB-model} both
#' arguments, \sQuote{\code{Amat}} and \sQuote{\code{Bmat}}, have to be set as
#' matrix objects containing desired restrictions and \sQuote{\code{NA}}
#' values. The parameters are estimated by minimising the negative of the
#' concentrated log-likelihood function:
#'
#' \deqn{ \ln L_c(A, B) = - \frac{KT}{2}\ln(2\pi) + \frac{T}{2}\ln|A|^2 -
#' \frac{T}{2}\ln|B|^2 - \frac{T}{2}tr(A'B'^{-1}B^{-1}A\tilde{\Sigma}_u) }
#'
#' Two alternatives are implemented for this: a scoring algorithm or direct
#' minimization with \code{optim()}. If the latter is chosen, the standard
#' errors are returned if \code{SVAR()} is called with \sQuote{\code{hessian =
#' TRUE}}.\cr
#'
#' If \sQuote{\code{start}} is not set, then \code{0.1} is used as starting
#' values for the unknown coefficients.\cr
#'
#' The reduced form residuals can be obtained from the above equation
#' \emph{via} the relation: \eqn{\bold{u}_t = A^{-1}B\bold{\varepsilon}_t},
#' with variance-covariance matrix \eqn{\Sigma_U = A^{-1}BB'A^{-1'}}.\cr
#'
#' Finally, in case of an overidentified SVAR, a likelihood ratio statistic is
#' computed according to:
#'
#' \deqn{ LR = T(\ln\det(\tilde{\Sigma}_u^r) - \ln\det(\tilde{\Sigma}_u)) \quad
#' , } with \eqn{\tilde{\Sigma}_u^r} being the restricted variance-covariance
#' matrix and \eqn{\tilde{\Sigma}_u} being the variance covariance matrix of
#' the reduced form residuals. The test statistic is distributed as
#' \eqn{\chi^2(nr - 2K^2 - \frac{1}{2}K(K + 1))}, where \eqn{nr} is equal to
#' the number of restrictions.
#'
#' @aliases SVAR print.svarest
#' @param x Object of class \sQuote{\code{varest}}; generated by
#' \command{VAR()}.
#' @param estmethod Character, either scoring for estimating the SVAR-model
#' with the scoring algorithm (default), or directly minimizing the negative
#' log-likelihood.
#' @param start Vector with starting values for the parameters to be optimised.
#' @param lrtest Logical, over-identification LR test, the result is set to
#' \command{NULL} for just-identified system.
#' @param max.iter Integer, maximum number of iteration (used if
#' \code{estmethod = "scoring"}).
#' @param conv.crit Real, convergence value of algorithm (used if
#' \code{estmethod = "scoring"}).
#' @param maxls Real, maximum movement of the parameters between two iterations
...
#' is returned:\cr
#'
#' \item{A}{If A- or AB-model, the matrix of estimated coefficients.}
#' \item{Ase}{The standard errors of \sQuote{\code{A}}.} \item{B}{If A- or
#' AB-model, the matrix of estimated coefficients.} \item{Bse}{The standard
#' errors of \sQuote{\code{B}}.} \item{LRIM}{For Blanchard-Quah estimation
#' \code{LRIM} is the estimated long-run impact matrix; for all other SVAR
#' models \code{LRIM} is \code{NULL}.} \item{Sigma.U}{The variance-covariance
#' matrix of the reduced form residuals times 100, \emph{i.e.}, \eqn{\Sigma_U =
#' A^{-1}BB'A^{-1'} \times 100}.} \item{LR}{Object of class
#' \sQuote{\code{htest}}, holding the Likelihood ratio overidentification
#' test.} \item{opt}{List object returned by \command{optim()}.}
#' \item{start}{Vector of starting values.} \item{type}{SVAR-type, character,
#' either \sQuote{A-model}, \sQuote{B-model} or \sQuote{AB-model}.}
#' \item{var}{The \sQuote{\code{varest}} object \sQuote{\code{x}}.}
#' \item{iter}{Integer, the count of iterations.} \item{call}{The \code{call}
#' to \command{SVAR()}.}
#' @author Bernhard Pfaff
#' @seealso \code{\link{VAR}}, \code{\link{SVEC}}, \code{\link{logLik}},
#' \code{\link{irf}}, \code{\link{fevd}}
#' @references Amisano, G. and C. Giannini (1997), \emph{Topics in Structural
#' VAR Econometrics}, 2nd edition, Springer, Berlin.
#'
...
#' Krätzig (editors), \emph{Applied Time Series Econometrics}, Cambridge
